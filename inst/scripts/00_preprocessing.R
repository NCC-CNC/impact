...
PMP <- read_sf(file.path(root, "pmp", "Parcels_20210531_Achievements_prjEA_RRupdate.shp")) %>%
  filter(!is.na(NAME)) %>%
...
# Forest
frst <- rast(file.path(root, "themes", "albers",
                       "CA_forest_VLCE_2015_forest_only_ha_proj_scale.tif"))

# Grassland
gras <- rast(file.path(root, "themes", "albers",
                        "AAFC_LU2015_comb_masked_by_Prairie_grassland_comb.tif"))

# Wetlands
wetl <- rast(file.path(root, "themes", "albers",
                       "Wetland_comb_proj_diss_90m_Arc.tif"))

# Rivers
rivr <- rast(file.path(root, "themes", "albers",
                       "grid_1km_water_linear_flow_length_1km.tif"))

# Lakes
laks <- rast(file.path(root, "themes", "albers",
                       "Lakes_CanVec_50k_ha.tif"))

# Shoreline
shrl <- rast(file.path(root, "themes", "albers",
                       "Shoreline.tif"))

# Climate
cfor <- rast(file.path(root, "themes", "albers",
                       "fwdshortestpath.tif"))

cref <- rast(file.path(root, "themes", "albers",
                       "NA_combo_refugia_sum45.tif"))

# Carbon
csta <- rast(file.path(root, "themes", "albers",
                       "Carbon_Mitchell_2021_t.tif")) %>%
  terra::crop(cfor)

cseq <- rast(file.path(root, "themes", "albers",
                       "Carbon_Potential_NFI_2011_CO2e_t_year.tif")) %>%
  terra::crop(cfor)

# Freshwater
fwat <- rast(file.path(root, "themes", "albers", "water_provision_2a_norm.tif"))

# Recreation
recr <- rast(file.path(root, "themes", "albers", "rec_pro_1a_norm.tif"))
...
SAR <- rast(file.path(root, "themes", "albers", "SAR_sum.tif"))

# Amphibian - IUCN
amph <- rast(file.path(root, "themes", "albers", "amph_sum.tif"))

# Bird - IUCN
bird <- rast(file.path(root, "themes", "albers", "bird_sum.tif"))

# Mammal - IUCN
mamm <- rast(file.path(root, "themes", "albers", "bird_sum.tif"))

# Reptile - ICUN
rept <- rast(file.path(root, "themes", "albers", "rept_sum.tif"))

# Species at risk - NatureServe Canada
SAR_NSC <- rast(file.path(root, "themes", "albers", "NSC_SARsum.tif"))

# Endemic species - NatureServe Canada
END_NSC <- rast(file.path(root, "themes", "albers", "NSC_ENDsum.tif"))

# Several species - NatureServe Canada
SPP_NSC <- rast(file.path(root, "themes", "albers", "NSC_SPPsum.tif"))
...
save(PMP, PMP_tmp, PMP_sub, PMP_feat_stack_mean, PMP_spp_stack_mean,
     file = file.path(root, "basedata.RData"))
